...
def emiyearquart(nyear,nqrt):

	fdir=os.getcwd()+"/Data/"
	
	print "   Loading file"
	fin=open(fdir+"/Poverty_EMI.csv","rU")
	frin=csv.reader(fin,delimiter=",")
	fdin=list(frin)
	print "   File loaded"

	fout=open(fdir+"/Poverty_EMI_Q"+str(nqrt)+"_"+nyear+".csv","wb")
	...
	for row in range(1,len(fdin)):
		...
		pdate=datetime.strptime(fdin[row][5],"%m/%d/%Y")
